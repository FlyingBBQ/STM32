...
void USBH_USR_Init(void)
{
  ...
  if (startup == 0 ) {
    ...
    LCD_LOG_SetHeader("USB OTG HS HID");
#else
    LCD_LOG_SetHeader("USB OTG FS HID");
#endif
...
void USBH_USR_EnumerationDone(void)
{
  ...
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, "If device supported:" );
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, "Press button to test...");
...
void USR_MOUSE_Init	(void)
{
  ...
  LCD_DisplayChar(MOUSE_WINDOW_Y + 1, MOUSE_WINDOW_X + 1, '+');
  x_loc  = 0;
  y_loc  = 0; 
  prev_x = 0;
  prev_y = 0;
